// This file defines the loading functionality for the knife detail page
...
export async function load({ params, fetch }) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to load knife: HTTP error ${response.status}`);
    }
    ...
    console.error('Error loading knife data:', error);
    return {
      item: null,
      error: error.message || 'Failed to load knife details',
      timeRemaining: null,
      initialBidAmount: 0
    };
  }
