<script>
  ...
  async function refreshItem() {
    try {
      ...
      console.log('Fetching painting details for ID:', id);
      ...
      console.log('Successfully loaded painting data:', data);
...
<svelte:head>
  <title>{item ? `${item.title} | Painting Auction` : 'Painting Auction Details'}</title>
  <meta name="description" content={item ? `Bid on ${item.title}, a painting by ${item.youtuber?.name || 'an Alaska YouTuber'}` : 'Painting auction details'} />
</svelte:head>
...
<div class="container mx-auto px-4 py-8">
  ...
  {:else if error}
    <div class="bg-red-900/30 border border-red-700 text-red-300 p-6 rounded-lg text-center max-w-2xl mx-auto">
      <h2 class="text-xl font-bold mb-2">Error Loading Painting</h2>
      <p>{error}</p>
      <button 
  ...
  {:else if item}
    <div class="grid grid-cols-1 xl:grid-cols-[3fr_2fr] gap-8">
      ...
      <div>
        ...
        <div class="mb-8 relative">
          <Enhanced3DCard
            ...
            badgeColor={timeRemaining && !timeRemaining.isExpired ? 'bg-indigo-600' : 'bg-neutral-700'}
          >
            <svelte:fragment slot="background">
              <div 
                class="absolute inset-0 bg-gradient-to-br from-indigo-900/60 to-violet-800/60 z-[{zValues.farBackground}]"
              ></div>
              <div 
                class="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(138,43,226,0.1),transparent_70%)] z-[{zValues.backgroundGlow}]"
                style="transform: translateZ({sineWave(currentTime, 5, 0.5)}px);"
              ></div>
              <div 
                class="absolute inset-0 bg-[linear-gradient(to_right,rgba(60,10,80,0.05)_1px,transparent_1px),linear-gradient(to_bottom,rgba(60,10,80,0.05)_1px,transparent_1px)] bg-[size:24px_24px] z-[{zValues.backgroundPattern}]"
                ...
                <div class="relative h-full w-full overflow-hidden rounded-lg">
                  ...
                  >
                    ...
                    <p class="text-indigo-200 text-sm">
                      By {item.youtuber?.name || 'Unknown Artist'}
                    </p>
                  </div>
                  ...
                  >
                    <span class="text-indigo-200 text-xs font-medium">Current bid:</span>
                  ...
                  {#if timeRemaining && !timeRemaining.isExpired}
                    <div
                      class="absolute top-3 left-3 bg-indigo-900/60 backdrop-blur-sm px-3 py-1.5 rounded-full z-[{zValues.countdownBox}]"
                      style="transform: translateZ({sineWave(currentTime + 0.2, 14, 0.3)}px);"
                    >
                      <span class="text-indigo-100 text-xs font-medium">Ends in:</span>
                      <span class="text-white font-bold ml-1">
      ...
      <div class="space-y-6">
        ...
        <div>
          <h1 class="text-3xl font-bold text-indigo-400">{item.title}</h1>
        ...
        <div class="bg-gray-900/50 backdrop-blur-sm rounded-lg p-5 border border-gray-800">
          <div class="flex justify-between mb-3">
            ...
            <span class="text-xl font-bold text-indigo-400">{formatPrice(item.current_price)}</span>
          </div>
          ...
          {#if timeRemaining && !timeRemaining.isExpired}
            <button
              ...
              class="w-full py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-medium rounded-md hover:from-indigo-500 hover:to-violet-500 transition-all"
            >
              Place Bid
            </button>
        ...
        <!-- Specifications -->
        <div class="bg-gray-900/50 backdrop-blur-sm rounded-lg p-5 border border-gray-800">
          <h2 class="text-xl font-semibold text-indigo-400 mb-4">About this Painting</h2>
          
          <div class="space-y-3">
            {#if item.medium}
              <div class="flex justify-between">
                <span class="text-gray-400">Medium:</span>
                <span class="text-gray-200">{item.medium}</span>
              </div>
            {/if}
            ...
            {#if item.style}
              <div class="flex justify-between">
                ...
                <span class="text-gray-200">{item.style}</span>
              </div>
            {/if}
            ...
            {#if item.year}
              <div class="flex justify-between">
                ...
                <span class="text-gray-200">{item.year}</span>
              </div>
            {/if}
            ...
            {#if item.artist}
              <div class="flex justify-between">
                ...
                <span class="text-gray-200">{item.artist}</span>
              </div>
            {/if}
          </div>
        ...
        <!-- Bid history -->
        <div class="bg-gray-900/50 backdrop-blur-sm rounded-lg p-5 border border-gray-800">
          <h2 class="text-xl font-semibold text-indigo-400 mb-4">Bid History</h2>
      ...
      {#if item.images.length > 1}
        <div class="absolute bottom-0 left-0 right-0 flex justify-center gap-2 p-4">
          {#each item.images as _, i}
            <button 
              class="w-3 h-3 rounded-full {i === popupImageIndex ? 'bg-indigo-500' : 'bg-gray-600 hover:bg-gray-500'}"
